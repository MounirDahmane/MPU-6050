...
void IMU_calibration()
{
  for(int i = 0; i < 3000; i++)
  {
    ...
    Wire.requestFrom(MPU_ADDR, 6, 0x43);  
  ...
  Gyro_X_error /= 3000.0; // ~999.00
  Gyro_Y_error /= 3000.0; // ~0.83
  Gyro_Z_error /= 3000.0; // ~0.39
  ...
  for(int i = 0; i < 3000; i++)
  {
    ...
    Accel_Y_error += (atan( (-1 *Accel_X) / sqrt( (pow(Accel_Y, 2) + pow(Accel_Z, 2)))) * RAD_TO_DEG);
    Accel_X_error += (atan( (Accel_Y) / sqrt( (pow(Accel_X, 2) + pow(Accel_Z, 2)))) * RAD_TO_DEG);
  }
  
  Accel_X_error /= 3000.0; // ~86.37
  Accel_Y_error /= 3000.0; // ~-0.25
}
...
void get_angle() 
{
  ...
  Acc_data();
      // euler angle formula
  Accel_Y_angle = (atan( (-1 *AccelX) / sqrt( pow(AccelY, 2) + pow(AccelZ, 2))) * RAD_TO_DEG) - Accel_Y_error;
  Accel_X_angle = (atan( (AccelY)    / sqrt( pow(AccelX, 2) + pow(AccelZ, 2))) * RAD_TO_DEG) - Accel_X_error;
